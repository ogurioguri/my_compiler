...
for testcase in test_file:
    try:
        ...
        temp.flush()
        commands = 'java -cp ~/Compiler-Design-Implementation-master/lib/antlr-4.13.2-complete:bin Main -emit-llvm '
        process = subprocess.Popen(commands, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
        ...
        process.terminate()
        commands = 'clang-15 -m32 src/output.ll src/util/buildin/builtin.ll -o test'
        process = subprocess.Popen(commands, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
